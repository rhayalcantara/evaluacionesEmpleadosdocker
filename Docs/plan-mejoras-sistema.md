...
| T6.1 | Dedup formatDateForInput | [x] | 2026-07-07 | (pendiente) |
| T6.2 | Tipar ComunicacionService (constantes) | [x] | 2026-07-07 | (pendiente) |
| T6.3 | Tipar getdatos<T> / ModelResponse<T> | [x] | 2026-07-07 | (pendiente) |
| T6.4 | loadLogoAsBase64 → UtilsService | [x] | 2026-07-07 | (pendiente) |
| T6.5 | Unificar cards (condicional a T1.5) | diferida | 2026-07-07 | (pendiente) |
...
- **T6.5** Unificación de cards (solo si T1.5 detectó uso activo de la generación vieja): migrar los usos de `card-empleado2`/`empleadoteam` a `card-empleado`/`empleadoteam2` template a template y borrar la vieja. Capturas antes/después.
  - **Resultado (2026-07-07): DIFERIDA, no ejecutada.** `empleadoteam2` (`EmpleadoTeamComponent2`) NO es un reemplazo directo de `empleadoteam`: está diseñado para abrirse como `MatDialog` (recibe sus datos via `@Inject(MAT_DIALOG_DATA)`, sin ningún `@Input()`), mientras que `home.component.html` usa `empleadoteam` embebido inline en la plantilla (`<app-empleadoteam [empleado]="empleado" [periodo]="periodo" [mostrarevaluacion]="false">`). Migrar Home implicaría o bien cambiar la UX de "lista inline en la página" a "modal emergente", o bien rediseñar `EmpleadoTeamComponent2` para soportar `@Input()` además del modo dialog — ninguna es un cambio "template a template" trivial, y el plan pide capturas antes/después que no puedo tomar en este entorno. Mismo criterio que T4.3/T5.3.
