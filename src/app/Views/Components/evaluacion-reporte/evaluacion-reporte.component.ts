...
export class EvaluacionReporteComponent implements OnInit {
  ...
  titulos: string[] = ['ID'           , 'Colaborador','Oficina','Inicio Contrato', 'Departamento', 'Posición', 'Supervisor', 'Estatus'           , 'Total Puntuación Objetivos'    ,'Resultado Objetivos (30%)','AutoEvaluacion Competencia     ' ,'Autoevaluación Competencia (70%)','Resultado Final AutoEvaluación (30% + 70%)','20% Resultado AutoEvaluación','Supervisor Competencia'         ,'Evaluacion del Supervisor (70%)' ,'Competencia Supervisor (30% + 70%)','80% Resultado Evaluacion Supervisor', 'Puntuacion General (20% + 80%)'];
  ...
  loadReportData() {
    ...
    this.evaluacionService.GetEvaluacionReporte01(this.currentPeriodId).subscribe({
      next: (response: ModelResponse) => {
        if (response && response.data) {
          this.reportData = (response.data as IReporte01[]).filter(e => !!e.colaborador);
          this.filteredData = [...this.reportData];
